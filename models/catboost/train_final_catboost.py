"""
CatBoost Benchmark Model Training, Cross-Validation, and Comparative Evaluation Pipeline.
...
Dataset:
- Primary Dataset v2: loan_evaluation_dataset_3000_v2.csv (500 Empirical + 2,500 Gaussian Copula synthetic records)
- Preprocessed: data/processed/ (X_train: 2,400 x 55, X_test: 600 x 55)

Module Purpose:
- Trains the CatBoost benchmark classifier on X_train (iterations=300, depth=6, lr=0.05, scale_pos_weight=1.1486).
- Evaluates on the holdout test set (N=600).
- Performs 5-fold Stratified Cross-Validation strictly on training data (N=2,400).
- Extracts native feature importances mapped with feature_names.json.
- Generates ROC and Precision-Recall curves.
- Compares Random Forest Baseline vs. XGBoost Proposed vs. CatBoost Benchmark.
- Persists all artifacts to models/catboost/ and evaluation/.
"""
...
def resolve_paths() -> Dict[str, Path]:
    ...
    models_dir = workspace_root / "models" / "catboost"
    ...
    return {
        ...
        "xgb_results": eval_dir / "final_xgboost_results.json",
        "model_output": models_dir / "final_catboost.joblib",
        "cm_output": eval_dir / "final_catboost_confusion_matrix.csv",
        "cr_output": eval_dir / "final_catboost_classification_report.json",
        "pred_output": eval_dir / "final_catboost_predictions.csv",
        "cv_output": eval_dir / "final_catboost_cross_validation.csv",
        "imp_output": eval_dir / "final_catboost_feature_importance.csv",
        "roc_output": eval_dir / "final_catboost_roc_curve.png",
        "pr_output": eval_dir / "final_catboost_precision_recall_curve.png",
        "results_json": eval_dir / "final_catboost_results.json",
        "comparison_v2_csv": eval_dir / "final_model_comparison_v2.csv",
    }


def run_pipeline():
    print("=" * 85)
    print("STEP 11: CATBOOST BENCHMARK MODEL (DATASET v2 — GAUSSIAN COPULA)")
    print("=" * 85)
    ...
    # 2. Train Model
    print(f"\n[1] Training CatBoost Benchmark Model (iterations={ITERATIONS}, depth={DEPTH}, lr={LEARNING_RATE}, scale_pos_weight={scale_pos_weight})...")
    ...
    # Save Model Artifact
    joblib.dump(cb_model, paths["model_output"])
    ...
    print("\n" + "-" * 50)
    print("FINAL CATBOOST TEST SET PERFORMANCE METRICS")
    print("-" * 50)
    ...
    # 7. 5-Fold Stratified Cross-Validation on Training Data ONLY
    ...
    for fold_idx, (t_idx, v_idx) in enumerate(skf.split(X_train, y_train), 1):
        ...
        fold_cb.fit(X_tr_f, y_tr_f)
        p_fold = fold_cb.predict(X_val_f)
        prob_fold = fold_cb.predict_proba(X_val_f)[:, 1]
    ...
    print("\n[4] Extracting native CatBoost feature importances...")
    imp_values = cb_model.get_feature_importance()
    df_imp = pd.DataFrame({
        "Feature": feature_names,
        "Importance": imp_values,
        "Normalized_Importance_%": (imp_values / imp_values.sum()) * 100,
    ...
    plt.plot(fpr, tpr, color="#2ca02c", lw=2, label=f"CatBoost Benchmark (AUC = {roc_auc:.4f})")
    ...
    plt.title("Receiver Operating Characteristic (ROC) Curve\nCatBoost Benchmark Model (Dataset v2, N=600)", fontsize=12, fontweight="bold", pad=15)
    ...
    plt.figure(figsize=(8, 6), dpi=300)
    plt.plot(rec_pts, prec_pts, color="#2ca02c", lw=2, label=f"CatBoost Benchmark (AP = {avg_prec:.4f})")
    ...
    results_summary = {
        ...
        "top_10_features": df_imp.head(10)[["Feature", "Importance"]].to_dict(orient="records"),
    }
    ...
    # 12. Final 3-Model Comparison (RF vs. XGBoost vs. CatBoost)
    print("\n[7] Generating 3-Model Comparison (Random Forest vs. XGBoost vs. CatBoost)...")
    if paths["rf_results"].exists() and paths["xgb_results"].exists():
        with open(paths["rf_results"], "r", encoding="utf-8") as f:
            rf_res = json.load(f)
        with open(paths["xgb_results"], "r", encoding="utf-8") as f:
            xgb_res = json.load(f)

        rf_tm = rf_res["test_metrics"]
        xgb_tm = xgb_res["test_metrics"]
        cb_tm = results_summary["test_metrics"]
        ...
        comp_records = [
            ...
            {
                "Model": "CatBoost (Benchmark Experiment)",
                "Accuracy": cb_tm["accuracy"],
                "Precision": cb_tm["precision"],
                "Recall": cb_tm["recall"],
                "F1-Score": cb_tm["f1_score"],
                "ROC-AUC": cb_tm["roc_auc"],
                "Average_Precision": cb_tm["average_precision"],
            },
            {
                "Model": "Delta (CatBoost - XGBoost)",
                "Accuracy": round(cb_tm["accuracy"] - xgb_tm["accuracy"], 4),
                "Precision": round(cb_tm["precision"] - xgb_tm["precision"], 4),
                "Recall": round(cb_tm["recall"] - xgb_tm["recall"], 4),
                "F1-Score": round(cb_tm["f1_score"] - xgb_tm["f1_score"], 4),
                "ROC-AUC": round(cb_tm["roc_auc"] - xgb_tm["roc_auc"], 4),
                "Average_Precision": round(cb_tm["average_precision"] - xgb_tm["average_precision"], 4),
            },
            {
                "Model": "Delta (CatBoost - Random Forest)",
                "Accuracy": round(cb_tm["accuracy"] - rf_tm["accuracy"], 4),
                "Precision": round(cb_tm["precision"] - rf_tm["precision"], 4),
                "Recall": round(cb_tm["recall"] - rf_tm["recall"], 4),
                "F1-Score": round(cb_tm["f1_score"] - rf_tm["f1_score"], 4),
                "ROC-AUC": round(cb_tm["roc_auc"] - rf_tm["roc_auc"], 4),
                "Average_Precision": round(cb_tm["average_precision"] - rf_tm["average_precision"], 4),
            },
        ]
        df_comp_v2 = pd.DataFrame(comp_records)
        df_comp_v2.to_csv(paths["comparison_v2_csv"], index=False, encoding="utf-8")
        print(f"[OK] Saved 3-Model Comparison to: {paths['comparison_v2_csv'].name}")
        print("\n--- FINAL 3-MODEL COMPARISON TABLE ---")
        print(df_comp_v2.to_string(index=False))

    print("\n" + "=" * 85)
    print("[SUCCESS] STEP 11 CATBOOST BENCHMARK EXECUTION COMPLETED")
    print("=" * 85)
